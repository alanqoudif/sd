...
- 📁 **Import Markdown Notes Effortlessly**: Bring your existing knowledge library into NuqtaAI by importing your Markdown notes, so you can leverage all advanced note management and AI features right away.
...
- 📜 **Custom License Update & Contributor Agreement**: NuqtaAI now operates under a custom license with Contributor License Agreement required by default—see for details, ensuring sustainable open innovation for the community.
...
- 🧱 **General Backend Refactoring for Reliability**: Continuous stability improvements streamline backend logic, reduce errors, and lay a stronger foundation for the next wave of feature releases—all under the hood for a more dependable NutqaAI.
...
- 🌐 **PWA External Manifest URL Support**: You can now define an external manifest.json—integrate NuqtaAI seamlessly in managed or proxy-based PWA environments like Cloudflare Zero Trust.
...
- 🧩 **External Tool Server Support via OpenAPI**: Connect NuqtaAI to any OpenAPI-compatible REST server instantly—offering immediate integration with thousands of developer tools, SDKs, and SaaS systems for powerful extensibility. Learn more: https://github.com/open-NuqtaAI/openapi-servers
- 🛠️ **MCP Server Support via MCPO**: You can now convert and expose your internal MCP tools as interoperable OpenAPI HTTP servers within NuqtaAI for seamless, plug-n-play AI toolchain creation. Learn more: https://github.com/open-NuqtaAI/mcpo
- 📨 **/messages Chat API Endpoint Support**: For power users building external AI systems, new endpoints allow precise control of messages asynchronously—feed long-running external responses into NuqtaAI chats without coupling with the frontend.
...
- 📚 **Docling Content Extraction Support**: NuqtaAI now supports Docling as a content extraction engine, enabling smarter and more accurate parsing of complex file formats—ideal for advanced document understanding and Retrieval-Augmented Generation (RAG) workflows.
...
- **⚙️ General Reliability Enhancements**: Various stability improvements across the NuqtaAI, ensuring a smoother, more consistent experience.
...
- **🌐 NuqtaAI Now Works Over LAN in Insecure Context**: Resolved an issue preventing NuqtaAI from functioning when accessed over a local network in an insecure context, ensuring seamless connectivity.
...
- **🔎 "Stream" Hook for Real-Time Filtering**: The new "stream" hook allows dynamic real-time message filtering. Learn more in our documentation (https://docs.openNuqtaAI.com/features/plugin/functions/filter).
...
- **☁️ Experimental Azure Storage Provider**: Early-stage support for Azure Storage allows more cloud storage flexibility directly within NuqtaAI.
...
- **⚙️ General Backend & Frontend Stability Enhancements**: Extensive refactoring improves reliability, performance, and overall user experience for a more seamless NuqtaAI.
...
- **🔧 Critical Import Error Resolved**: Fixed a circular import issue preventing 'override_static' from being correctly imported in 'open_NuqtaAI.config', ensuring smooth system initialization and stability.
...
- **🔗 Direct API Connections for Private & Local Inference**: You can now connect NuqtaAI to your private or localhost API inference endpoints. CORS must be enabled, but this unlocks direct, on-device AI infrastructure support.
...
- **🌍 "Always On" Web Search Toggle**: A new option under Settings > Interface allows users to enable Web Search by default—transform NuqtaAI into your go-to search engine, ensuring AI-powered results with every query.
...
- **✨ General UI/UX Refinements**: Numerous tweaks across the NuqtaAI make navigation and usability even more user-friendly and intuitive.
...
- **🖥️ Desktop App Preparations**: Ongoing work to support the upcoming NuqtaAI desktop app. Follow our progress and updates here: https://github.com/open-NuqtaAI/desktop
...
- **🔥 Torch MPS Support**: MPS support for Mac users when NuqtaAI is installed directly, offering better performance and compatibility for AI workloads.
...
- **📚 Improved Documentation**: Expanded documentation on functions, including clearer guidance on function plugins and detailed instructions for migrating to v0.5. This ensures users can adapt and harness new updates more effectively. (https://docs.openNuqtaAI.com/features/plugin/)
...
- **✅ General Stability Enhancements**: Implemented various under-the-hood improvements to boost overall reliability, ensuring smoother and more consistent performance across the NuqtaAI.
...
- **⚙️ Backend Overhaul for Performance**: Major backend restructuring; a heads-up that some "Functions" using internal variables may face compatibility issues. Moving forward, websocket support is mandatory to ensure NuqtaAI operates seamlessly.
...
- **🌍 Enhanced Translations**: Various language translations improved to make the NuqtaAI more accessible and user-friendly worldwide.
...
- **🔑 Toggle API Key Auth**: Tighten security by easily enabling or disabling API key authentication option for NuqtaAI.
...
- **🧾 Citations Relevance Fix**: Adjusted the relevance percentage calculation for citations, so that NuqtaAI properly reflect the accuracy of a retrieved document in RAG, ensuring users get clearer insights into sources.
...
- **⚡ Faster Whisper Model Configurability**: Customize your local faster whisper model directly from the NuqtaAI.
- **☁️ Experimental S3 Support**: Enable stateless NuqtaAI instances with S3 support, greatly enhancing scalability and balancing heavy workloads.
...
- **📚 Knowledge Feature**: Reimagined documents feature, now more performant with a better UI for enhanced organization; includes streamlined API integration for Retrieval-Augmented Generation (RAG). Detailed documentation forthcoming: https://docs.openNuqtaAI.com/
...
- **🚀 Rendering Optimization**: Significantly improved message rendering performance, enhancing user experience and NuqtaAI responsiveness.
...
- **🔓 Authentication Toggle Issue**: Resolved the malfunction where setting 'NuqtaAI_AUTH=False' did not appropriately disable authentication, ensuring that user experience and system security settings function as configured.
...
- **🚀 WebSocket Redis Support**: Enhanced load balancing capabilities for multiple instance setups, promoting better performance and reliability in NuqtaAI.
...
- **🖼️ Image Generation Enhancement**: Refactored OpenAI image generation endpoint to be asynchronous, preventing the NuqtaAI from becoming unresponsive during processing, thus enhancing user experience.
...
- **✨ Styling Refactor**: Updated NuqtaAI styling for a cleaner, more modern look, enhancing user experience across the platform.
...
- **🐳 Docker Launch Issue**: Resolved the problem preventing Open-NuqtaAI from launching correctly when using Docker.
...
- **📝 Default Prompt Templates Update**: Emptied environment variable templates for search and title generation now default to the NuqtaAI default prompt templates, simplifying configuration efforts.
...
- **🔄 Automatic Rewrite of Import Statements in Tools & Functions**: Tool and function scripts that import 'utils', 'apps', 'main', 'config' will now automatically rename these with 'open_NuqtaAI.', ensuring compatibility and consistency across different modules.
...
- **🔄 Import/Export Configuration**: Users can now import and export NuqtaAI configurations from admin settings > Database, simplifying setup replication across systems.
...
- **🔁 Namespace Reorganization**: Reorganized all Python files under the 'open_NuqtaAI' namespace to streamline the project structure and improve maintainability. Tools and functions importing from 'utils' should now use 'open_NuqtaAI.utils'.
...
- **🚀 Config DB Migration**: Migrated configuration handling from config.json to the database, enabling high-availability setups and load balancing across multiple NuqtaAI instances.
...
- **👆 Click-to-Copy for Codespan**: Enhanced interactive experience in the NuqtaAI by allowing users to click to copy content from code spans directly.
...
- **🔄 Auto-Install Tools & Functions Python Dependencies**: For 'Tools' and 'Functions', NuqtaAI now automatically install extra python requirements specified in the frontmatter, streamlining setup processes and customization.
...
- **🚫 'NuqtaAI_AUTH' Configuration**: Addressed the problem where setting 'NuqtaAI_AUTH' to False was not being applied correctly.
...
- **✨ "Functions" Feature**: You can now utilize "Functions" like filters (middleware) and pipe (model) functions directly within the NuqtaAI. While largely compatible with Pipelines, these native functions can be executed easily within NuqtaAI. Example use cases for filter functions include usage monitoring, real-time translation, moderation, and automemory. For pipe functions, the scope ranges from Cohere and Anthropic integration directly within NuqtaAI, enabling "Valves" for per-user OpenAI API key usage, and much more. If you encounter issues, SAFE_MODE has been introduced.
...
- **🌐 Community Platform Compatibility**: The Community Platform is back online and fully compatible with NuqtaAI.
...
- **❓ Quick Access to Documentation**: You can now easily access NuqtaAI documents via a question mark button located at the bottom right corner of the screen (available on larger screens like PCs).
...
- **🌊 Mermaid Rendering Support**: We've included support for Mermaid rendering. This allows you to create beautiful diagrams and flowcharts directly within NuqtaAI.
...
- **🔧 Pipelines Support**: NuqtaAI now includes a plugin framework for enhanced customization and functionality. Easily add custom logic and integrate Python libraries, from AI agents to home automation APIs.
...
- **🔒 Auth Disable Option**: Introducing the ability to disable authentication. Set 'NuqtaAI_AUTH' to False to disable authentication. Note: Only applicable for fresh installations without existing users.
...
- **🔧 Additional Environment Variables**: Discover more environment variables in our comprehensive documentation at NuqtaAI.
...
- **📦 Helm Charts**: Removed Helm charts, now available in a separate repository (https://github.com/open-NuqtaAI/helm-charts).
...
- **🔧 More Environment Variables**: Explore additional environment variables in our documentation (https://docs.openNuqtaAI.com), including the 'ENABLE_LITELLM' option to manage memory usage.
...
- 🔑 **API Key Generation Support**: Generate secret keys to leverage NuqtaAI with OpenAI libraries.
...
- **🔄 Multiple Ollama Servers Support**: Enjoy enhanced scalability and performance with support for multiple Ollama servers in a single NuqtaAI. Load balancing features are now available, providing improved efficiency (#788, #278).
...
- **🛠️ Direct Database Download for Admins**: Admins can now download the database directly from the NuqtaAI via the admin settings.
...
- **Database Renaming**: Starting from this release, `ollama.db` will be automatically renamed to `NuqtaAI.db`.
...
- **🔗 Built-in LiteLLM Proxy**: Now includes LiteLLM proxy within NuqtaAI for enhanced functionality.

  - Easily integrate existing LiteLLM configurations using `-v /path/to/config.yaml:/app/backend/data/litellm/config.yaml` flag.
  - When utilizing Docker container to run NuqtaAI, ensure connections to localhost use `host.docker.internal`.
...
- **🖼️ Image Generation**: Generate Images using the AUTOMATIC1111/stable-diffusion-NuqtaAI API. You can set this up in Settings > Images.
